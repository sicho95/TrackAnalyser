...
test('crée un participant puis enregistre et analyse hors cloud', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Choisir le fond de carte' }).click()
  await page.getByRole('menuitemradio', { name: 'OpenTopoMap relief' }).click()
  await expect(page.locator('.maplibregl-ctrl-top-right .map-style-control')).toBeVisible()
...
test('persiste les paramètres de détection des segments', async ({ page }) => {
  await page.getByRole('link', { name: /Réglages/ }).click()
  const similarity = page.getByRole('slider', { name: /Similarité minimale/ })
  const minimumLength = page.getByRole('spinbutton', { name: /Longueur minimale/ })
  await similarity.fill('94')
  await minimumLength.fill('250')
  await page.getByRole('button', { name: 'Appliquer et recalculer' }).click()
  await expect(page.getByText('Réglages enregistrés et segments recalculés.')).toBeVisible()
  await page.reload()
  await expect(similarity).toHaveValue('94')
  await expect(minimumLength).toHaveValue('250')
})
