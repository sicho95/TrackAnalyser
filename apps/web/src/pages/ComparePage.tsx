...
export function ComparePage(): ReactNode {
  ...
  const routeSegments = segments.filter((segment) => !segment.manual)
  const effectiveLeftSegmentId = routeSegments.some((segment) => segment.id === leftSegmentId) ? leftSegmentId : routeSegments[0]?.id ?? ''
  const leftSegment = routeSegments.find((segment) => segment.id === effectiveLeftSegmentId)
  const comparableSegments = leftSegment === undefined ? [] : routeSegments.filter((segment) => segmentsAreComparable(leftSegment, segment))
  ...
  return <div className="screen"><ScreenHeader eyebrow={messages.compare.eyebrow} title={messages.compare.title} />
    ...
    {mode === 'SEGMENT' ? <section className="segment-picker"><Route size={20} /><div className="comparison-picker"><EntityPicker label={messages.compare.segmentA} value={effectiveLeftSegmentId} onChange={setLeftSegmentId} options={routeSegments.map((segment) => ({ id: segment.id, label: segmentLabel(segment, sessions, participants, equipment) }))} /><EntityPicker label={messages.compare.segmentB} value={effectiveRightSegmentId} onChange={setRightSegmentId} options={comparableSegments.map((segment) => ({ id: segment.id, label: segmentLabel(segment, sessions, participants, equipment) }))} /></div><p>{leftSegment === undefined ? messages.compare.segmentEmpty : leftSegment.routeFingerprint === undefined ? messages.compare.segmentContext : messages.compare.segmentGps}</p></section> : null}
