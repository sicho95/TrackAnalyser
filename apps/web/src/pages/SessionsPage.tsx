...
export function SessionsPage(): ReactNode {
  ...
  return (
    <div className="screen">
      <ScreenHeader eyebrow={`${sessions.length} session${sessions.length > 1 ? 's' : ''}`} title={messages.sessions.title} action={<label className="icon-button" aria-label={messages.sessions.importAria}><FileInput size={21} /><input type="file" accept=".fit,.gpx,.tcx,.json,.xml,.tatrip" onChange={(event) => void selectFile(event)} hidden /></label>} />
