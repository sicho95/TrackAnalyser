...
export function SessionsPage(): ReactNode {
  ...
  return (
    <div className="screen">
      <ScreenHeader eyebrow={`${sessions.length} session${sessions.length > 1 ? 's' : ''}`} title="Sessions" action={<label className="icon-button" aria-label="Importer un fichier"><FileUp size={21} /><input type="file" accept=".fit,.gpx,.tcx,.json,.xml" onChange={(event) => void selectFile(event)} hidden /></label>} />
