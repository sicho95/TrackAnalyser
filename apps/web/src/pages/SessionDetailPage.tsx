...
import { createTripArchive, exportSamplesCsv, exportSummaryJson } from '@track-analyser/exporters'
...
export function SessionDetailPage(): ReactNode {
  ...
  const exportCsv = (): void => download(exportSamplesCsv([]), `${session.id}.csv`, 'text/csv')
  const exportTrip = (): void => download(createTripArchive({ session, analysisRuns: runs, samples: [], rawFiles: {} }), `${session.id}.tatrip`, 'application/zip')
  ...
  return <div className="screen detail-screen"><ScreenHeader eyebrow={`${participant?.name ?? ''} · ${session.activityType}`} title={session.title ?? new Date(session.startTime).toLocaleDateString('fr-FR')} action={<Link to="/sessions" className="icon-button"><ArrowLeft size={20} /></Link>} />
    ...
    <section className="export-actions"><button type="button" onClick={exportJson}><Download size={18} />JSON</button><button type="button" onClick={exportCsv}><Download size={18} />CSV</button><button type="button" onClick={exportTrip}><Download size={18} />.tatrip</button></section>
