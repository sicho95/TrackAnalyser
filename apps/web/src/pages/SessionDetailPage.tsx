...
import { ArrowLeft, Database, Download, GitCompareArrows, RefreshCw, Route, Trash2, Upload } from 'lucide-react'
...
export function SessionDetailPage(): ReactNode {
  ...
  const routeSegments = current?.result.routePreviewSegments ?? (route.length === 0 ? [] : [route])
  const sessionSegments = segments.filter((segment) => segment.sessionId === session.id && !segment.manual)
  const currentProfile = profiles.find((profile) => profile.version === current?.analysisProfileVersion)
  const maximumContinuityGapMs = Math.max(1, currentProfile?.parameters.maximumContinuousGapSeconds ?? 60) * 1_000
  const analysisStateMessage = session.analysisStatus === 'RUNNING' ? messages.detail.analysisRunning : session.analysisStatus === 'PENDING' ? messages.detail.analysisPending : session.analysisStatus === 'FAILED' ? messages.detail.analysisFailed : messages.detail.noAnalysis

  const runExport = async (kind: 'JSON' | 'CSV' | 'GPX' | 'TATRIP'): Promise<void> => exportSession(session, runs, sessionSegments, kind, maximumContinuityGapMs)
  ...
  return <div className="screen detail-screen"><ScreenHeader eyebrow={`${participant?.name ?? ''} · ${session.activityType}`} title={session.title ?? new Date(session.startTime).toLocaleDateString('fr-FR')} action={<Link to="/sessions" className="icon-button"><ArrowLeft size={20} /></Link>} />
    ...
    {route.length === 0 ? null : <Suspense fallback={<div className="map-placeholder">{messages.detail.loadingMap}</div>}><MapView route={route} segments={routeSegments} provider={settings.mapProvider} onProviderChange={(mapProvider) => void updateSettings({ ...settings, mapProvider })} /></Suspense>}
    {current === undefined ? <section className="analysis-state"><RefreshCw size={20} /><p>{analysisStateMessage}{session.analysisError === undefined ? null : <small>{session.analysisError}</small>}</p></section> : <><section className="quality-strip"><div><strong>{Math.round(current.result.quality.confidence * 100)} %</strong><span>{messages.common.confidence}</span></div><div><strong>{Math.round(current.result.quality.coverage * 100)} %</strong><span>{messages.common.coverage}</span></div><div><strong>{current.analysisVersion}</strong><span>{messages.common.engine}</span></div></section>{current.result.quality.coverage < 0.95 ? <section className="analysis-warning"><strong>{messages.detail.incompleteRecording}</strong><p>{messages.detail.incompleteRecordingBody.replace('{coverage}', Math.round(current.result.quality.coverage * 100).toString())}</p></section> : null}<div className="metric-grid">{current.result.metrics.filter((metric) => metric.status === 'AVAILABLE').map((metric) => <MetricPanel key={metric.id} metric={metric} values={series[metric.id.split('.')[0] ?? ''] ?? []} />)}</div>{current.result.events.length === 0 ? null : <section className="events-panel"><h2>{messages.detail.events}</h2>{current.result.events.map((event) => <div key={event.id}><strong>{event.type}</strong><span>{new Date(event.startTime).toLocaleTimeString('fr-FR')}</span></div>)}</section>}</>}
    ...
    <section className="export-actions"><button type="button" onClick={() => void runExport('JSON')}><Upload size={18} />JSON</button><button type="button" onClick={() => void runExport('CSV')}><Upload size={18} />CSV</button><button type="button" onClick={() => void runExport('GPX')}><Upload size={18} />GPX</button><button type="button" onClick={() => void runExport('TATRIP')}><Upload size={18} />.tatrip</button></section>
