...
import { ArrowLeft, Database, FileInput, FileOutput, GitCompareArrows, RefreshCw, Route, Trash2 } from 'lucide-react'
...
export function SessionDetailPage(): ReactNode {
  ...
  const runExport = async (kind: 'JSON' | 'CSV' | 'GPX' | 'TATRIP'): Promise<void> => exportSession(session, runs, sessionSegments, kind)
  ...
  return <div className="screen detail-screen"><ScreenHeader eyebrow={`${participant?.name ?? ''} · ${session.activityType}`} title={session.title ?? new Date(session.startTime).toLocaleDateString('fr-FR')} action={<Link to="/sessions" className="icon-button"><ArrowLeft size={20} /></Link>} />
    ...
    <section className="export-actions"><button type="button" onClick={() => void runExport('JSON')}><FileOutput size={18} />JSON</button><button type="button" onClick={() => void runExport('CSV')}><FileOutput size={18} />CSV</button><button type="button" onClick={() => void runExport('GPX')}><FileOutput size={18} />GPX</button><button type="button" onClick={() => void runExport('TATRIP')}><FileOutput size={18} />.tatrip</button></section>
