...
La V1 expose huit parcours : session, participant, équipement, ActivityGroup, segment reconnu, événements comparables, évolution temporelle et versions d’analyse. Les agrégations participant/équipement restent filtrées par activité. Les séries d’un segment persistant sont normalisées sur leur propre durée puis affichées sur les mêmes axes, sans modifier les données sources.
